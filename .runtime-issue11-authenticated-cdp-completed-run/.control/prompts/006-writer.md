...
Assemble candidate 001 to `drafts/article-001.md` in this isolated workspace by applying the
...
The prose draft has 1678 such characters; the assembled article must have fewer. Take the reduction from the
explanation each visual now carries rather than from unrelated sections.

Planned image paths, staged read-only at the same relative path under `context/`:

- visuals/article-001/assets/shot-001.png (opportunity opp-003, alt text "Screenshot of the example.com page: a bold heading reading Example Domain, below it one short paragraph reading This domain is for use in documentation examples without needing permission. Avoid use in operations. and below that a Learn more link, all on a plain light grey background.")
...
## Provided context: drafts/article-001-prose.md

<write-uuter-context name="drafts/article-001-prose.md">
...
Three things are visible on the destination page. The heading reads "Example
Domain". Below it, a single short paragraph states that the domain is for use in
documentation examples without needing permission, and advises against use in
operations. Below that sits a "Learn more" link pointing to fuller detail on
IANA's site.

That paragraph is the whole answer to the question. The page grants the
permission it is describing, sets one limit on it — keep it out of anything
operational — and offers a link for readers who want more.
...
## Provided context: visual-plan.json

<write-uuter-context name="visual-plan.json">
{
  "schema_version": 1,
  "source_revision": "sha256:2182d470bbe1c6dc15061f05fbe23e0f842c1e3651de129442cb7053742fcc67",
  "opportunities": [
    {
      "id": "opp-001",
